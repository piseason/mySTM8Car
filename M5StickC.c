...
//カルマン・フィルタ処理 
#define theta_update_interval 0.002     //2msec,500Hz
...
void angleCorrection()
{
  ...
  thetaOffset = fmaxf(-3.0, fminf(3.0, thetaOffset));	//在没有速度控制时，判断角度误差需要时间，太大就倒了，也修正不过来；太快就振荡了，和速度控制类似了
}
...
//    theta = theta_data[0][0]; theta_dot = theta_dot2;
    ...
    //PID部分,单位化
    thetaP = (theta_data[0][0]-thetaOffset-thetaV) / 100.0;
